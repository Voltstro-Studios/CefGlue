...
using NativeMethods = Xilium.CefGlue.Platform.Windows.NativeMethods;
...
public static unsafe class CefRuntime
{
        ...
        if (!string.IsNullOrEmpty(path))
        {
            if (Platform == CefRuntimePlatform.Windows)
                ...
                throw new PlatformNotSupportedException(
                    "CEF Runtime can't be initialized on altered path on this platform. Use CefRuntime.Load() instead.");
        }
    ...
    public static bool RegisterExtension(string extensionName, string javascriptCode, CefV8Handler? handler)
    {
        ...
        fixed (char* javascriptCode_str = javascriptCode)
        {
            ...
            return libcef.register_extension(&n_extensionName, &n_javascriptCode,
                handler != null ? handler.ToNative() : null) != 0;
        }
    }
    ...
    public static string GetPath(CefPathKey pathKey)
    {
        ...
        if (!success) throw new InvalidOperationException($"Failed to get path for key {pathKey}");
        return value!;
    }
    ...
    /// </summary>
    public static bool AddCrossOriginWhitelistEntry(string sourceOrigin, string targetProtocol, string? targetDomain,
    ...
    /// </summary>
    public static bool RemoveCrossOriginWhitelistEntry(string sourceOrigin, string targetProtocol, string? targetDomain,
        bool allowTargetSubdomains)
    {
        if (string.IsNullOrEmpty("sourceOrigin")) throw new ArgumentNullException(nameof(sourceOrigin));
        if (string.IsNullOrEmpty("targetProtocol")) throw new ArgumentNullException(nameof(targetProtocol));
    ...
    /// </summary>
    public static bool RegisterSchemeHandlerFactory(string schemeName, string? domainName,

...
{
    private int on_file_dialog(cef_dialog_handler_t* self, cef_browser_t* browser, CefFileDialogMode mode,
        ...
        int selected_accept_filter, cef_file_dialog_callback_t* callback)
    {
        ...
        var result = OnFileDialog(mBrowser, mode, mTitle, mDefaultFilePath, mAcceptFilters, selected_accept_filter,
            mCallback);
    ...
    protected virtual bool OnFileDialog(CefBrowser browser, CefFileDialogMode mode, string? title,
        string? defaultFilePath, string[] acceptFilters, int selectedAcceptFilter, CefFileDialogCallback callback)
    {
        return false;
    }
}
...
public sealed unsafe class CefMainArgs
{
    ...
    private cef_main_args_t_windows* ToNativeWindows()
    {
        ...
        ptr->instance = NativeMethods.GetModuleHandle(null);
        return ptr;
    }
    ...
    private IntPtr MarshallArgcArgvBlock(string[] args)
    {
        ...
        foreach (var arg in args) size += 1 + encoding.GetByteCount(arg ?? "");

        var argv = (byte**) Marshal.AllocHGlobal(size);
        var data = (byte*) argv + sizeOfArray;
